...
func miniMax(depth int, tree Tree, player Color,
	...
	if player == Self { //maximizer
		...
		for i := 0; i < len(tree.nodes); i++ {
			...
			if First(tree.board.movePiece(tree.nodes[i].oldPos, tree.nodes[i].newPos)).check(User) == 1 && val > tree.board.evaluate() {
				best = val
	...
	for i := 0; i < len(tree.nodes); i++ {
		...
		//experimental - for risk taking
		if First(tree.board.movePiece(tree.nodes[i].oldPos, tree.nodes[i].newPos)).check(Self) == 1 && val < tree.board.evaluate() {
			best = val
